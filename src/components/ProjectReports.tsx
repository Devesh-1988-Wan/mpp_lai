...
import { Task, TaskPriority, TaskStatus, TaskType, DocsProgressStatus } from "@/types/project";
...
export function ProjectReports({ tasks, onExportReport }: ProjectReportsProps) {
    const analytics = useMemo(() => {
        ...
        const totalResources = tasks.reduce((acc, task) => acc + (task.num_resources || 0), 0);
        const totalHoursAvailable = tasks.reduce((acc, task) => acc + (task.total_hours_available || 0), 0);
        const uniqueReleaseVersions = [...new Set(tasks.map(t => t.release_version).filter(Boolean))];
        ...
        <div className="space-y-6">
                        ...
                        <div>
                            ...
                            <div className="flex flex-wrap gap-2 mt-2">
                                {Object.entries(analytics.tasksByPriority).map(([priority, count]) => (
                                    <Badge key={priority} style={getPriorityBadgeStyle(priority as TaskPriority)}>{priority}: {count}</Badge>
                                ))}
                            </div>
                        </div>
                        <div className="w-12 h-12 bg-primary/10 rounded-full flex items-center justify-center">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <Card className="p-6">
                    <div className="flex items-center justify-between">
                        <div>
                            <p className="text-sm font-medium text-muted-foreground">Total Hours Available</p>
                            <p className="text-3xl font-bold">{analytics.totalHoursAvailable}</p>
                        </div>
                        <div className="w-12 h-12 bg-primary/10 rounded-full flex items-center justify-center">
                ...
                <Card className="p-6">
                    <div className="flex items-center justify-between">
                        <div>
                            <p className="text-sm font-medium text-muted-foreground">Release Versions</p>
                            <p className="text-3xl font-bold">{analytics.uniqueReleaseVersions.length}</p>
                        </div>
                        <div className="w-12 h-12 bg-primary/10 rounded-full flex items-center justify-center">
